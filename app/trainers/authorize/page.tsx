...
function TrainerAuthorizeContent() {
  ...
  const handleAuthorize = () => {
    setAuthorizing(true)
    window.location.href = '/api/lark/auth/authorize'
  }
  ...
  const handleRevoke = async (email: string) => {
    ...
    try {
      const response = await fetch('/api/trainers/revoke-authorization', {
        method: 'POST',
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          {loading ? (
            ...
            <div>
              <h2 className="text-lg font-semibold text-[#0b0707] mb-4">
                Authorized Trainers ({trainers.filter(t => t.authorized).length})
              </h2>
              
              {trainers.length === 0 ? (
                <p className="text-[#6b6a6a] text-center py-8">
                  No trainers authorized yet
                </p>
              ) : (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {trainer.calendarId && trainer.calendarId !== 'primary' && (
          ...
          <div className="mt-8 p-4 bg-blue-50 rounded-xl">
            <p className="text-sm text-blue-800">
              <strong>Note:</strong> Each trainer must authorize individually using their own Lark account. 
              Share this page URL with trainers who need to connect their calendars.
            </p>
          </div>
        </div>
      </div>
